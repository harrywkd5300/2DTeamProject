...
void CTileMgr::Update()
{
	...
	int iCullEndX = g_iMaxSizeX / TILECX;
	int iCullEndY = g_iMaxSizeY / TILECY;
...
{
	// WINAPI에서 제공하는 파일 개방 함수.
	HANDLE hFile = CreateFile(L"../Data/Stage2.dat", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS
		, FILE_ATTRIBUTE_NORMAL, NULL);
...
void CTileMgr::Scroll()
{
	...
	if(WINCX - (TILECX * TILEX)  > g_iScrollX)
		g_iScrollX = WINCX - (TILECX * TILEX);

	if(0 < g_iScrollY)
		g_iScrollY = 0;

	if(WINCY - (TILECY * TILEY)  > g_iScrollY)
		g_iScrollY = WINCY - (TILECY * TILEY);
}